...
# 1) Set up
# 2) Analysis
#     (a) washington population counts
...
##### USCIS DATA #####
# Read in USCIS data
washington <- temp %>% select("Region.Name", "country_birth", "WASHINGTON") %>% 
...
acs_WA20 <- data %>% subset(YEAR == 2020 & STATEFIP == 53)
acs_WA20_imm <- acs_WA20 %>% subset(BPL > 120)
acs_WA20_natz <- acs_WA20 %>% subset(CITIZEN == 2) 
acs_WA20_newNatz <- acs_WA20_natz %>% subset(YRNATUR >=2016)

###### ANALYSIS ###### 
#============================================================================
# WASHINGTON POPULATION COUNTS
#============================================================================
###### WALCULATE NUMBER OF IMMIGRANTS, TOTAL NATURALIZED IN washington ######
sum(acs_WA20$PERWT) # washington population
sum(acs_WA20_imm$PERWT) # number of immigrants in washington
sum(acs_WA20_natz$PERWT) # number of naturalized citizens in washington
sum(acs_WA20_newNatz$PERWT)  # number of newly naturalized citizens in washington

####### WALCULATE NUMBER ELIGIBLE FOR CITIZENSHIP ####################
eligible_citizen <- acs_WA20_imm %>% subset(CITIZEN %in% c(3, 4) & YRSUSA1 > 5)
sum(eligible_citizen$PERWT)

####### PRE-TAX WAGE CONTRIBUTIONS ####################
# get rid of 99999/99998 values representing N/A or Missing incomes
incwage <- (acs_WA20_imm %>% subset(INCWAGE < 999998))
...
n_newnatz = 86909
natz_by_region <- as.data.frame(washington %>% group_by(Region.Name) %>% summarise(Freq = sum(WASHINGTON)) %>%
                                  mutate(Percentage = Freq/n_newnatz))
View(natz_by_region)

# top ten country of origins
View(washington %>% arrange(desc(washington)))
...
# WA
WA <- met_region %>% filter(grepl("WA", MET2013_NAME))
View(as.data.frame(WA))
...
#============================================================================
####### WALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN washington ###########
#============================================================================
# workforce populaiton in washington
n_washington = sum(acs_WA20$PERWT[acs_WA20$OCC != 0])
occ_general <- (
  acs_WA20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_washington)
)

# immigrant workforce population
n_imm = sum(acs_WA20_imm$PERWT[acs_WA20_imm$OCC != 0])
occ_imm <- (
  acs_WA20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(washington, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_WA20, "out/acs_WA20.csv", row.names = F)
write.csv(acs_WA20_imm, "out/acs_WA20_imm.csv", row.names = F)
write.csv(acs_WA20_natz, "out/acs_WA20_natz.csv", row.names = F)
write.csv(acs_WA20_newNatz, "out/acs_WA20_newNatz.csv", row.names = F)
