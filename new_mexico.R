...
##### USCIS DATA #####
# Read in USCIS data
new_mexico <- temp %>% select("Region.Name", "country_birth", "NEW.MEXICO") %>% 
...
# Subsets: ACS Data - New Mexico,  ACS Data - Immigrants, ACS Data - Naturlized, 
#           ACS Data - Newly Naturalized
acs_NM20 <- data %>% subset(YEAR == 2020 & STATEFIP == 35)
acs_NM20_imm <- acs_NM20 %>% subset(BPL > 120)
acs_NM20_natz <- acs_NM20 %>% subset(CITIZEN == 2) 
acs_NM20_newNatz <- acs_NM20_natz %>% subset(YRNATUR >=2016)

###### ANALYSIS ###### 
#============================================================================
# NM POPULATION COUNTS
#============================================================================
###### CALCULATE NUMBER OF IMMIGRANTS, TOTAL NATURALIZED IN New Mexico ######
sum(acs_NM20$PERWT) # New Mexico population
sum(acs_NM20_imm$PERWT) # number of immigrants in New Mexico
sum(acs_NM20_natz$PERWT) # number of naturalized citizens in New Mexico
sum(acs_NM20_newNatz$PERWT)  # number of newly naturalized citizens in New Mexico

####### CALCULATE NUMBER ELIGIBLE FOR CITIZENSHIP ####################
eligible_citizen <- acs_NM20_imm %>% subset(CITIZEN %in% c(3, 4) & YRSUSA1 > 5)
sum(eligible_citizen$PERWT)

####### PRE-TAX WAGE CONTRIBUTIONS ####################
# get rid of 99999/99998 values representing N/A or Missing incomes
incwage <- (acs_NM20_imm %>% subset(INCWAGE < 999998))
...
n_newnatz = 15064
natz_by_region <- as.data.frame(new_mexico %>% group_by(Region.Name) %>% summarise(Freq = sum(NEW.MEXICO)) %>%
                                  mutate(Percentage = Freq/n_newnatz))
View(natz_by_region)

# top ten country of origins
View(new_mexico %>% arrange(desc(new_mexico)))

# look for new_mexico's position relative to other state's naturalizations in each region
...
# NM
NM <- met_region %>% filter(grepl("NM", MET2013_NAME))
View(as.data.frame(NM))
...
#============================================================================
####### WALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN New Mexico ###########
#============================================================================
# workforce populaiton in New Mexico
n_new_mexico = sum(acs_NM20$PERWT[acs_NM20$OCC != 0])
occ_general <- (
  acs_NM20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_new_mexico)
)

# immigrant workforce population
n_imm = sum(acs_NM20_imm$PERWT[acs_NM20_imm$OCC != 0])
occ_imm <- (
  acs_NM20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(new_mexico, "Country_Region_Birth_NM.csv", row.names = F)
write.csv(county_region, "Natz_by_County_NM.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_NM.csv", row.names = F)
write.csv(acs_NM20, "out/acs_NM20.csv", row.names = F)
write.csv(acs_NM20_imm, "out/acs_NM20_imm.csv", row.names = F)
write.csv(acs_NM20_natz, "out/acs_NM20_natz.csv", row.names = F)
write.csv(acs_NM20_newNatz, "out/acs_NM20_newNatz.csv", row.names = F)
