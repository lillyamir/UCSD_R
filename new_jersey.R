...
# 1) Set up
# 2) Analysis
#     (a) new jersey population counts
...
##### USCIS DATA #####
# Read in USCIS data
newjersey <- temp %>% select("Region.Name", "country_birth", "NEW.JERSEY") %>% 
...
acs_NJ20 <- data %>% subset(YEAR == 2020 & STATEFIP == 34)
acs_NJ20_imm <- acs_NJ20 %>% subset(BPL > 120)
acs_NJ20_natz <- acs_NJ20 %>% subset(CITIZEN == 2) 
acs_NJ20_newNatz <- acs_NJ20_natz %>% subset(YRNATUR >=2016)

###### ANALYSIS ###### 
#============================================================================
# new jersey POPULATION COUNTS
#============================================================================
###### NJLCULATE NUMBER OF IMMIGRANTS, TOTAL NATURALIZED IN new jersey ######
sum(acs_NJ20$PERWT) # new jersey population
sum(acs_NJ20_imm$PERWT) # number of immigrants in new jersey
sum(acs_NJ20_natz$PERWT) # number of naturalized citizens in new jersey
sum(acs_NJ20_newNatz$PERWT)  # number of newly naturalized citizens in new jersey

####### NJLCULATE NUMBER ELIGIBLE FOR CITIZENSHIP ####################
eligible_citizen <- acs_NJ20_imm %>% subset(CITIZEN %in% c(3, 4) & YRSUSA1 > 5)
sum(eligible_citizen$PERWT)

####### PRE-TAX WAGE CONTRIBUTIONS ####################
# get rid of 99999/99998 values representing N/A or Missing incomes
incwage <- (acs_NJ20_imm %>% subset(INCWAGE < 999998))
...
n_newnatz = 177890
natz_by_region <- as.data.frame(newjersey %>% group_by(Region.Name) %>% summarise(Freq = sum(NEW.JERSEY)) %>%
                                  mutate(Percentage = Freq/n_newnatz))
View(natz_by_region)

# top ten country of origins
View(newjersey %>% arrange(desc(newjersey)))
...
# NJ
nj <- met_region %>% filter(grepl("NJ", MET2013_NAME))
View(as.data.frame(nj))
...
#============================================================================
####### NJLCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN new jersey ###########
#============================================================================
# workforce populaiton in new jersey
n_newjersey = sum(acs_NJ20$PERWT[acs_NJ20$OCC != 0])
occ_general <- (
  acs_NJ20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_newjersey)
)

# immigrant workforce population
n_imm = sum(acs_NJ20_imm$PERWT[acs_NJ20_imm$OCC != 0])
occ_imm <- (
  acs_NJ20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(newjersey, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_NJ20, "out/acs_NJ20.csv", row.names = F)
write.csv(acs_NJ20_imm, "out/acs_NJ20_imm.csv", row.names = F)
write.csv(acs_NJ20_natz, "out/acs_NJ20_natz.csv", row.names = F)
write.csv(acs_NJ20_newNatz, "out/acs_NJ20_newNatz.csv", row.names = F)
